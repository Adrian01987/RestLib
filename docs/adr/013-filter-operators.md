...
- Portable EF query expressions are covered by SQLite behavioral tests, SQL Server provider-translation smoke tests, and a live Windows LocalDB matrix using explicit case-sensitive and case-insensitive collations. The live matrix verifies the portable ASCII/literal baseline under both collations and separately asserts the documented provider-dependent equality and case-sensitive-search behavior.
