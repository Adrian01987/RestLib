...
/// via individual <see cref="Abstractions.IRepository{TEntity, TKey}.DeleteAsync"/> calls.
/// Does not use a bulk path to ensure per-item 404 detection.
/// </summary>
    ...
    where TKey : notnull
{
    ...
    /// <inheritdoc/>
    protected override bool HasBulkPath => false;
        ...
        BatchContext<TEntity, TKey> context)
    {
        ...
        if (!deleted)
        {
            ...
            results[index] = new BatchItemResult
            {
                ...
                Error = Responses.ProblemDetailsFactory.NotFound(entityName, key!, context.HttpContext.Request.Path)
            };
            return;
        }
